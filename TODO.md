...
- `[DONE 2026-08-23]` **Avoid redundant bar redraws.** All three guards shipped and benchmarked flat (focus same-client == different-client; updatestatus identical-text early return; setlayout same-layout no dirty).

- `[DONE 2026-08-23]` **Avoid redundant `arrange()` calls.** Implemented as event-loop coalescing (`arrange_pending` + `flusheventtail()`); grabs/setup stay immediate. Measured -66% per request in bursts. All 25+ callers invoke `arrange()` unconditionally — no check for whether client geometry or layout state actually changed. Add guard conditions at hot call sites (tag, fullscreen, swallow, unmanage, floating toggles) or introduce an `arrange_pending` coalescing flag (mirroring `bar_draw_pending`) to defer to the event-loop tail.

- - `[DONE 2026-08-23]` **Reduce status update churn.** updatestatus() compares new text vs stext before dirtying; fullscreen skip already active via optimizefullscreen guard. External status scripts can fire `XA_WM_NAME` changes on root at high frequency. `updatestatus()` runs on every one with no debouncing, rate-limiting, or before/after `stext` comparison. Consider: comparing new text vs. old before dirtying segments; skipping entirely when fullscreen is focused; or capping to ~10 Hz via a timestamp guard.
...
- `[DONE]` **Skip `updatestatus()` during fullscreen.** (optimizefullscreen guard at top of updatestatus). When the bar is frozen by `optimizefullscreen`, processing status updates is wasted work. Add a guard at the top of `updatestatus()` or in `propertynotify()`: if `optimizefullscreen && selmon->sel && selmon->sel->isfullscreen`, return early.

- `[DONE 2026-08-23]` **`propertynotify` early atom filtering.** Uninteresting atoms return before wintoclient(); measured 13->4 ns (-69%). Every property change on root or any client dispatches through `propertynotify()`, which calls `XGetWindowProperty` for interesting atoms. Return early for uninteresting atoms with a cheap constant-time check before any X11 call.

- `[DONE 2026-08-23, pivoted]` **Cache visible-client counts in tile/monocle.** Count cache rejected (tile counts nexttiled, not ISVISIBLE); replaced by single-pass tiled snapshot in tile(). -2..-4% on all-tiled lists, more with floating prefix. Both `tile()` and `monocle()` walk the full client list on every arrange to count visible clients. Cache this count and update it incrementally on attach/detach/showhide to avoid O(n) walks on every layout pass.

- `[REJECTED 2026-08-23]` **Single-monitor `enternotify` guard.** 0% measurable + AGENTS.md gotcha: the branch is required for focus-follows-mouse. Same pattern as `motionnotify`: on single-monitor setups, `EnterWindowMask` events are delivered but `enternotify()` (dwm.c:694) still runs through the full `wintoclient`/`wintomon`/`focus` chain. Either conditionally mask the event, or add a fast-path return when `!mons->next`.
